...
contract Dailylimit is BasePlugin, IDailylimit, SafeLock {
    ...
    bytes4 private constant FUNC_EXECUTE =
        bytes4(keccak256("execute(address,uint256,bytes)"));
    bytes4 private constant FUNC_EXECUTE_BATCH =
        bytes4(keccak256("executeBatch(address[],bytes[])"));
    bytes4 private constant FUNC_EXECUTE_BATCH_VALUE =
        bytes4(keccak256("executeBatch(address[],uint256[],bytes[])"));
    bytes4 private constant FUNC_EXEC_FROM_MODULE =
        bytes4(keccak256("execFromModule(bytes)"));
    ...
    constructor()
        BasePlugin(keccak256("PLUGIN_DAILYLIMIT_SLOT"))
    ...
    function _init(bytes calldata data) internal override onlyDelegateCall {
        ...
        if (l.currentDay == 0) {
            ...
            // decode data
            (address[] memory tokens, uint256[] memory limits) = abi.decode(
                data,
                (address[], uint256[])
            );
    ...
    function _decodeERC20Spent(
        bytes4 selector,
        address to,
        bytes memory data
    ) private view returns (address token, uint256 spent) {
    ...
    function _decodeExecute(
        ...
        if (dataLength > 4 && l.tokens.isExist(to)) {
            ...
            (address token, uint256 spent) = _decodeERC20Spent(
                selector,
                to,
                data
            );
            ...
            if (spent > 0) {
                ...
                require(
                    ...
                    "Dailylimit: token daily limit reached"
                );
            }
        }
    }
    ...
    function guardHook(
        ...
        if (_validationData > 0) {
            ...
            require(
                validationData.validUntil - validationData.validAfter <
                    MAX_TIMERANGE,
    ...
    function postHook(
        ...
        if (l.tokens.isExist(target)) {
            (, uint256 _spent) = _decodeERC20Spent(
                bytes4(data[0:4]),
                target,
                data[4:]
            );
